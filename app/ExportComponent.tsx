...
export default function ExportComponent({ geoJsonData, selectedCoordinateSystem, expedientCode }) {
  ...
  return (
    <div className="flex flex-col justify-center gap-4">
      ...
      >
        {isExportingSHP ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Exportando...</> : 'Exportar SHP'}
      </Button>
      <Button
        variant="default"
        className="w-full bg-green-500 text-white"
        ...
        disabled={isExportingSHP || isExportingKML}
      >
        {isExportingKML ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Exportando...</> : 'Exportar KML'}
      </Button>
    </div>
  )
}
